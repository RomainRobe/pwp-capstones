...
def get_average_sentence_length(text):
    text2=text.replace('!','.')
    text3=text2.replace('?','.')
    sentence_text = text3.split('.')
    counter = 0
    for sentence in sentence_text:
        for letter in sentence:
            counter  += 1
    average = counter / len(sentence_text)
    return average
...
def prepared_text(text):
    ...
    for words in text.lower().split():
        text_splited.append(words.strip().strip(punctuation))
    return text_splited
...
def build_frequency_table(corpus):
    ...
    for element in corpus:
        ...
        elif element in frequency_table:
            counter += 1
            frequency_table[element] = counter
...
def ngram_creator(text_list):
    ...
    for i in range(len(text_list)-1):
        ...
        mylist= [a,b]
        ngram.append(mylist)
    for portion in ngram:
        ng = portion[0] + " " + portion[1]
        ngram_bis.append(ng)
    return ngram_bis
...
def frequency_comparison(table1, table2):
    ...
    for word in table1.keys():
        if word in table2.keys():
            if table2[word] > table1[word]:
                appearances += table2[word]
                mutual_appearances += table1[word]
            elif table1[word] >= table2[word]:
                appearances += table1[word]
                mutual_appearances += table2[word]
        elif word not in table2.keys():
            appearances += table1[word]
...
class TextSample:
    ...
    def __repr__(self):
        return '{author} wrote {length} in her introduction.'.format(author = self.author, length = self.average_sentence_length)

murderer_sample = TextSample(murder_note, 'Killer')
...
#print(prepared_text(lily_trebuchet_intro))
print("Lily Trebuchet has a score of {}".format(find_text_similarity(murder_note, lily_trebuchet_intro)))
print("Myrtle Beech has a score of {}".format(find_text_similarity(murder_note, myrtle_beech_intro)))
print("Gregg T Fishy has a score of {}".format(find_text_similarity(murder_note, gregg_t_fishy_intro)))
